...
\date{Last updated: \today \\ \small DOI / Reference: \url{https://doi.org/10.5281/zenodo.22982412} \cite{tran2026extended}}
...
\begin{document}
	...
	\begin{definition}[Cardinality Operator]
		Let $A$ be a classical set. The \textbf{cardinality} (or cardinal number) of $A$, denoted by $\mathrm{card}(A)$ or $|A|$, represents the measure of the number of elements in $A$. For any configuration $S^* \in \mathrm{Conf}(S)$, $\mathrm{card}(S^*)$ maps $S^*$ to its non-negative cardinal measure in $\mathbb{N} \cup \{\infty\}$.
	\end{definition}
	
	\begin{definition}[Uncertain Cardinality]
		Let $S = \{x_i : i \in I\}$ be an uncertain set on the universal set $U$. The \textbf{uncertain cardinality} (or number of uncertain elements) of $S$, denoted by $|S|$ or $\mathrm{card}(S)$, is an uncertain number on the set of natural numbers $\mathbb{N}$ whose possibility domain is defined by:
		\[
		\mathrm{Val}(|S|) = \mathrm{Val}(\mathrm{card}(S)) = \{ \mathrm{card}(S^*) : S^* \in \mathrm{Conf}(S) \} \subseteq \mathbb{N}
		\]
		where $\mathrm{card}(S^*) = |S^*|$ represents the classical cardinality of the configuration $S^*$.
	\end{definition}
	...
	\begin{example}[Numerical Illustration and Uncertain Cardinality]
		...
		\begin{itemize}
			\item When $x_1 = 1, x_2 = 1 \implies S_1^* = \{1, 1\} = \{1\}$ (due to element deduplication in classical set theory), implying $|S_1^*| = \mathrm{card}(S_1^*) = 1$.
			\item When $x_1 = 1, x_2 = 3 \implies S_2^* = \{1, 3\}$, implying $|S_2^*| = \mathrm{card}(S_2^*) = 2$.
			\item When $x_1 = 2, x_2 = 1 \implies S_3^* = \{2, 1\}$, implying $|S_3^*| = \mathrm{card}(S_3^*) = 2$.
			\item When $x_1 = 2, x_2 = 3 \implies S_4^* = \{2, 3\}$, implying $|S_4^*| = \mathrm{card}(S_4^*) = 2$.
		\end{itemize}
		...
		\]
		Hence, the uncertain cardinality $|S|$ (or $\mathrm{card}(S)$) has a possibility domain that constitutes an uncertain number on $\mathbb{N}$:
		\[
		\mathrm{Val}(|S|) = \{ |S^*| : S^* \in \mathrm{Conf}(S) \} = \{1, 2\} \subseteq \mathbb{N}
		\]
	\end{example}
	\begin{definition}[Element Membership in an Uncertain Set]
		Let $U$ be a non-empty universal set, and let $S = \{x_i : i \in I\}$ be an uncertain set on $U$.
		
		1. For a classical element $u \in U$, the assertion $u \in_u S$ (read as ``$u$ belongs to the uncertain set $S$'') is a multi-state logical proposition whose possibility domain of truth value is defined through the configuration space $\mathrm{Conf}(S)$:
		\[
		\mathrm{Val}(u \in_u S) = \{ (u \in S^*) : S^* \in \mathrm{Conf}(S) \} \subseteq \{\text{True}, \text{False}\}
		\]
		where $(u \in S^*)$ denotes the classical element membership assertion.
		
		2. For an uncertain element $y$ on $U$ with possibility domain $\mathrm{Val}(y) \subseteq U$, the assertion $y \in_u S$ is defined via joint configuration collapse:
		\[
		\mathrm{Val}(y \in_u S) = \{ (v \in S^*) : v \in \mathrm{Val}(y), \, S^* \in \mathrm{Conf}(S) \} \subseteq \{\text{True}, \text{False}\}
		\]
		
		We say that $u \in_u S$ (or $y \in_u S$) is \textbf{certainly true} if $\mathrm{Val}(\cdot) = \{\text{True}\}$, \textbf{certainly false} if $\mathrm{Val}(\cdot) = \{\text{False}\}$, and \textbf{strictly uncertain} if $\mathrm{Val}(\cdot) = \{\text{True}, \text{False}\}$.
	\end{definition}
	
	\begin{thebibliography}{9}
		\bibitem{tran2026extended}
		Tran, M. C. (2026). \textit{Extended logic and mathematics of uncertainty}. Zenodo. DOI: \href{https://doi.org/10.5281/zenodo.22982412}{10.5281/zenodo.22982412}.
		\bibitem{mu}
